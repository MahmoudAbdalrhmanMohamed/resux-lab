...
if (!(await exists(functionsDir))) {
  ...
  console.log("[resux-lab] No Vercel function output detected; production-report hotfix skipped.");
  process.exit(0);
}
...
for (const entry of functionEntries) {
  const targetDir = path.join(functionsDir, entry.name, ".resux");
  await mkdir(targetDir, { recursive: true });
  await copyFile(source, path.join(targetDir, "halal-report.json"));
}

console.log(
  `[resux-lab] Copied the signed Resux production report into ${functionEntries.length} Vercel function runtime(s).`,
);
